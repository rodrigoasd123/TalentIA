...
def test_benchmark_langgraph_tiene_ocho_nodos(monkeypatch) -> None:
    monkeypatch.setattr(benchmark, "build_llm_for_model", lambda store, model: _FakeLLM(model))
    monkeypatch.setattr(benchmark.MLFLOW_TRACKER, "record_benchmark_summary", lambda summary: None)
    engine = benchmark.build_model_benchmark_graph(prefer_langgraph=True)
    result = engine.invoke(
        {
            "store": object(),
            "models": ["gemini-3.6-flash"],
            "cases": list(benchmark.DEFAULT_CASES),
    ...
    assert result["ranking"][0]["gate_passed"] is True


def test_benchmark_no_expone_respuestas(monkeypatch) -> None:
    monkeypatch.setattr(benchmark, "build_llm_for_model", lambda store, model: _FakeLLM(model))
    monkeypatch.setattr(benchmark.MLFLOW_TRACKER, "record_benchmark_summary", lambda summary: None)
    result = benchmark.run_model_benchmark(
        object(), ["gemini-3.6-flash"], baseline_model="gemini-3.6-flash"
    )
    assert result["suite_version"] == benchmark.SUITE_VERSION
    assert result["suite_hash"]
