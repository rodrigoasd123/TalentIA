...
def candidate_to_model(candidate: Candidate) -> CandidateModel:
    return CandidateModel(
        ...
        reported_age=candidate.reported_age,
        bgc=candidate.bgc,
...
def candidate_to_entity(model: CandidateModel) -> Candidate:
    return Candidate(
        **_base_fields(model),
        ...
        client=model.client,
        candidate_status=CandidateStatus(model.candidate_status),
        record_date=model.record_date,
        recruiter=model.recruiter,
        q=model.q,
        birth_date=model.birth_date,
        reported_age=model.reported_age,
        bgc=model.bgc,
        technical_knowledge=model.technical_knowledge,
        equifax_debt=model.equifax_debt,
        salary_expectation=model.salary_expectation,
        requested=model.requested,
        role_ctc=model.role_ctc,
...
def evaluation_to_entity(model: EvaluationModel) -> Evaluation:
    return Evaluation(
        **_base_fields(model),
        ...
        dimension_scores=[_dimension_to_entity(d) for d in (model.dimension_scores or [])],
        ...
        bias_audit=(BiasAuditResult.model_validate(model.bias_audit) if model.bias_audit else None),
...
def workflow_to_entity(model: WorkflowRunModel) -> WorkflowRun:
    return WorkflowRun(
        **_base_fields(model),
        ...
        node_runs=list(model.node_runs or []),
