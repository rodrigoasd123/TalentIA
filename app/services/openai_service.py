...
class OpenAIService:
    ...
    def get_langchain_model(self, task: str = None, model: str = None, temperature: float = None):
        ...
        if not model:
            model = get_model_for_task(task) if task else self.settings.openai_model
    ...
    def call_model(
        ...
        while retries <= max_retries:
            try:
                ...
                # No reintentar
                self._save_usage_log({
                    "model": model, "feature": feature, "success": False,
                    "error_message": "API key inválida o ausente.", "latency_ms": int((time.time() - start_time) * 1000)
                })
                ...
                if "quota" in error_msg or "billing" in error_msg or "balance" in error_msg:
                    ...
                    raise OpenAIServiceError("No hay saldo disponible en la cuenta API de OpenAI. Revisa Billing en platform.openai.com.") from e
                
                if retries == max_retries:
                    self._save_usage_log({
                        "model": model, "feature": feature, "success": False,
                        "error_message": f"Rate limit: {e}", "latency_ms": int((time.time() - start_time) * 1000)
                    })
                ...
                if retries == max_retries:
                    self._save_usage_log({
                        "model": model, "feature": feature, "success": False,
                        "error_message": str(e), "latency_ms": int((time.time() - start_time) * 1000)
                    })
